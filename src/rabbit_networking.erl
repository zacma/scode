...
%% rabbit_sup -> mrabbit_tcp_sup 
start_listener0(Address, Protocol, Label, OnConnect) ->
    io:format("tcp address ~p portocol=~p Label=~p OnConnect=~p~n", [Address, Protocol, Label, OnConnect]),
	Spec = tcp_listener_spec(rabbit_tcp_listener_sup, Address, tcp_opts(),
                             Protocol, Label, OnConnect),
    case supervisor:start_child(mrabbit_tcp_sup, Spec) of
        {ok, _}                -> ok;
...
tcp_listener_started(Protocol, IPAddress, Port) ->
	...
	io:format("no mnesia OnStarted Protocol=~p IPAddress=~p Port=~p~n", 
...
tcp_listener_stopped(Protocol, IPAddress, Port) ->
	...
	io:format("no mnesia OnStopped Protocol=~p IPAddress=~p, Port=~p~n", 
						[Protocol, IPAddress, Port]).
	%% no mnesia
...
io:format(" new client Sock=~p, SockTransform=~p~n", [Sock, SockTransform]),
    {ok, _Child, Reader} = supervisor:start_child(rabbit_tcp_client_sup, []),
io:format(" Reader ~n"),
    ok = rabbit_net:controlling_process(Sock, Reader),
io:format("new client here~n"),	
...
tcp_opts() ->
    {ok, Opts} = application:get_env(tcp_listen_options),
    Opts.
